...
interface PluginSettings {
	...
	timezoneOffset: number; // hours, applied on top of system time
	startHour: number;
	endHour: number;
...
const DEFAULT_SETTINGS: PluginSettings = {
	...
	timezoneOffset: 0, // 0 = use device time as-is
	startHour: 9,
	endHour: 21,
...
export default class TelegramNotifierPlugin extends Plugin {
		...
		this.registerEvent(
			this.app.workspace.on(
				...
				(menu: Menu, editor: Editor) => {
					...
					menu.addItem((item) => {
						item
							.setTitle('🔔 TG: Insert Single Reminder')
							.setIcon('calendar')
							.onClick(() => {
								const fields = this.generateYamlFields(
									this.settings.allowedFieldsSingle,
								);
					...
					menu.addItem((item) => {
						item
							.setTitle('✅ TG: Insert Inline Task')
					...
					this.settings.presets.forEach((preset) => {
						menu.addItem((item) => {
							item
								.setTitle(
									`🔄 TG: Insert Preset '${preset.name}'`,
								)
								.setIcon('refresh-cw')
								.onClick(() => {
									const fields = this.generateYamlFields(
										this.settings.allowedFieldsPreset,
									);
									const snippet = `---\ndue_date: ${moment()
										.add(
											this.settings.timezoneOffset,
											'hours',
										)
										.format(
											'YYYY-MM-DD',
	...
	updateStatusBar(text: string) {
		if (this.statusBarItem)
			this.statusBarItem.setText(`🤖 TG: ${text}`);
	}

	getCurrentTime(): string {
		return moment()
			.add(this.settings.timezoneOffset, 'hours')
	...
	async checkReminders() {
		...
		if (
			currentHour < this.settings.startHour ||
			currentHour >= this.settings.endHour
		) {
		...
		// 3. Cleanup History (optional – kept simple)
		...
		if (cache.frontmatter) {
			...
			if (fm.review_date) {
				...
				if (targetTime && targetTime.isValid()) {
					const msg = this.formatTemplate(
						this.settings.defaultReviewTemplate,
						file,
						fm,
						'',
						this.settings.allowedFieldsSingle,
			...
			if (fm.due_date && fm.reminder_preset) {
				...
				if (baseTime && baseTime.isValid() && preset) {
					for (const offset of preset.offsets) {
						...
						await this.tryNotify(
							file,
							`preset_${fm.reminder_preset}_${offset}`,
							triggerTime,
							now,
							msg,
						);
			...
			if (match) {
				...
				if (targetTime && targetTime.isValid()) {
					const taskText = line
						...
						.trim();
					const msg = this.formatTemplate(
						this.settings.defaultInlineTemplate,
						file,
						{ task: taskText },
						'',
						'task',
...
class TelegramNotifierSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.createEl('h2', {
			text: 'Telegram Notifier Setup',
		});

		// --- Connection ---
		const connDiv = containerEl.createDiv();
		connDiv.style.border =
			'1px solid var(--background-modifier-border)';
		connDiv.style.padding = '10px';
		...
		new Setting(connDiv)
			...
			.addText((t) =>
				t
					...
					.onChange(async (v) => {
						this.plugin.settings.botToken = v;
		...
		new Setting(connDiv)
			...
			.addButton((b) =>
				b
					.setButtonText('Send Test Message 🚀')
					.onClick(async () => {
						if (!this.plugin.settings.botToken) return;
						await this.plugin.sendTelegram(
							'Ping! Connection OK.',
						);
						new Notice('Sent!');
					}),
		...
		containerEl.createEl('p', {
			text: 'Notifications will only be sent within this range. If date is removed from note, notifications stop.',
		});

		new Setting(containerEl)
			.setName('Check Interval (min)')
			.addSlider((s) =>
				s
					.setLimits(1, 60, 1)
					...
					.setDynamicTooltip()
					.onChange(async (v) => {
						this.plugin.settings.checkIntervalMinutes = v;
						await this.plugin.saveSettings();
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Notifications use your device’s local time as the base. Leave this at 0 to match your system time exactly. Set a positive or negative number if you want the plugin to use a different timezone than your device (for example, -3 or 2).",
			)
			.addText((t) =>
				t
					.setValue(
						String(this.plugin.settings.timezoneOffset),
					)
					.onChange(async (v) => {
						this.plugin.settings.timezoneOffset =
							Number(v) || 0;
						await this.plugin.saveSettings();
					}),
			);
		...
		new Setting(containerEl)
			...
			.setDesc('Start Hour (0-23) — End Hour (0-23)')
			.addText((t) =>
				t
					.setPlaceholder('Start')
					.setValue(String(this.plugin.settings.startHour))
					.onChange(async (v) => {
						this.plugin.settings.startHour = Number(v);
						await this.plugin.saveSettings();
					}),
			)
			.addText((t) =>
				t
					...
					.onChange(async (v) => {
						this.plugin.settings.endHour = Number(v);
						await this.plugin.saveSettings();
					}),
			);
		...
		// --- Configuration ---
		containerEl.createEl('h3', {
			text: '⚙️ Fields & Templates',
		});

		new Setting(containerEl)
			.setName('Allowed Fields (Single)')
			.setDesc(
				'YAML fields for Single Reminders (comma separated). Will be auto-inserted.',
			)
			.addTextArea((t) => {
				t.setValue(this.plugin.settings.allowedFieldsSingle)
					.onChange(async (v) => {
						this.plugin.settings.allowedFieldsSingle = v;
						await this.plugin.saveSettings();
					});
		...
		new Setting(containerEl)
			.setName('Allowed Fields (Preset)')
			.setDesc('YAML fields for Recurring Presets.')
			.addTextArea((t) => {
				t.setValue(this.plugin.settings.allowedFieldsPreset)
					.onChange(async (v) => {
						this.plugin.settings.allowedFieldsPreset = v;
						await this.plugin.saveSettings();
					});
				t.inputEl.style.width = '100%';
			});
		...
		new Setting(containerEl)
			.setName('Standard Review Template')
			.addTextArea((t) => {
				t.setValue(this.plugin.settings.defaultReviewTemplate)
					.onChange(async (v) => {
						this.plugin.settings.defaultReviewTemplate = v;
						await this.plugin.saveSettings();
					});
				t.inputEl.style.width = '100%';
			});
		...
		new Setting(containerEl)
			.setName('Inline Task Template')
			.addTextArea((t) => {
				t.setValue(this.plugin.settings.defaultInlineTemplate)
					.onChange(async (v) => {
						this.plugin.settings.defaultInlineTemplate = v;
						await this.plugin.saveSettings();
					});
				t.inputEl.style.width = '100%';
			});
		...
		new Setting(containerEl)
			...
			.addTextArea((t) => {
				t.setValue(
					...
					try {
						this.plugin.settings.presets = JSON.parse(v);
						this.jsonError = '';
						await this.plugin.saveSettings();
						t.inputEl.style.border = '1px solid green';
						setTimeout(() => this.display(), 500);
					} catch (e: any) {
						this.jsonError = e.message;
						t.inputEl.style.border = '1px solid red';
					}
				});
				t.inputEl.style.width = '100%';
				t.inputEl.style.height = '150px';
		...
		const genDiv = containerEl.createDiv();
		genDiv.style.padding = '15px';
		genDiv.style.backgroundColor =
		...
		if (this.genType === 'recurring') {
			new Setting(genDiv)
				...
				.addDropdown((dd) => {
					...
					if (
						!this.genPreset &&
						this.plugin.settings.presets.length > 0
					) {
						this.genPreset =
							this.plugin.settings.presets[0].name;
					}
					dd.setValue(this.genPreset);
					dd.onChange((v) => {
						this.genPreset = v;
						this.display();
					});
		...
		if (this.genType === 'single') {
			const fields = this.plugin.generateYamlFields(
				this.plugin.settings.allowedFieldsSingle,
			);
			snippet = `---\nreview_date: ${time.format(
				'YYYY-MM-DD HH:mm',
			)}\n${fields}---`;
		} else if (this.genType === 'recurring') {
			const fields = this.plugin.generateYamlFields(
				this.plugin.settings.allowedFieldsPreset,
			);
			snippet = `---\ndue_date: ${time.format(
				'YYYY-MM-DD',
			)}\nreminder_preset: ${
				this.genPreset || 'finance'
			}\n${fields}---`;
		} else if (this.genType === 'inline') {
			snippet = `- [ ] Task [check:: ${time.format(
				'YYYY-MM-DD HH:mm',
			)}]`;
		}
		...
		outArea.style.width = '100%';
		outArea.style.height = '100px';
		outArea.style.fontFamily = 'monospace';

		const btnRow = genDiv.createDiv();
